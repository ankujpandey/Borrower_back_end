...
class Users_repository {
  ...
  async getUserDataAgent(req) {
    try {
      ...
      let [len, meta] = await sequelize.query(
        `SELECT COUNT(*) as length From users INNER JOIN loan_details ON users.uid = loan_details.uid INNER JOIN JobAssignees ON JobAssignees.jobAssignees_id = loan_details.jobAssignees_id WHERE JobAssignees.jobAssignees_id = ${agentId} AND users.isDeleted = False`
      );
      ...
      const [data, metadata] = await sequelize.query(
        `SELECT users.uid, users.email,users.isActive,user_infos.contact, user_infos.pan, user_infos.aadhaar, user_infos.firstName,user_infos.lastName, loan_details.Loan_state, JobAssignees.name AS AgentName FROM users INNER JOIN user_infos ON users.uid = user_infos.uid INNER JOIN loan_details ON users.uid = loan_details.uid INNER JOIN JobAssignees ON JobAssignees.jobAssignees_id = loan_details.jobAssignees_id WHERE JobAssignees.jobAssignees_id = ${agentId} AND users.isDeleted = False LIMIT ${limit} OFFSET ${skip}`
      );
      reqObj.data = data;
      reqObj.length = len;
      return reqObj;
  ...
  async getUserDataEmail(id) {
    try {
      ...
      console.log("Something went wrong in user repository layer".magenta);
      throw { error };
    }
  }
}
